...
int main(int argc, char* argv[]){
  ...
  for (int i=0; i < stoi(argv[2]); i++) {
    ...
    f_sum += all_g[i].frontiers.size();
    for (auto v : all_g[i].frontiers) {
      pq.push(make_pair(all_g[i].graph[v].indegree, v));
    }
    int tnum = omp_get_max_threads();
    int q = ceil(float(all_g[i].frontiers.size())/float(tnum));
  ...
  cout << "Total Number of Frontiers: " << f_sum << endl;
  //return 0;

  // Compute PPR of v
  int s, s_gm=0;
  vector<float> ppr(max_vsize);
  vector< pair<int, float> > current(max_vsize);
  vector< pair<int, float> > next(max_vsize);

  // choose source node randomly
  for (int i = 0; i < all_g[s_gm].vsize; i++) {
    //if (all_g[s_gm].graph[i].outdegree > 1 && all_g[s_gm].graph[i].indegree > 1) {
  ...
  float thr = stof(argv[5]);
  vector<float> attacked(max_vsize);
  float s_value = 0;
  while (all_remain > thr) {
    float tmp_remain = 0;
    // all current scan
    //float debug_count=0;
    //#pragma omp parallel for reduction(+:s_value, tmp_remain, debug_count)
    #pragma omp parallel for reduction(+:s_value, tmp_remain)
    for (int i = 0; i < max_vsize; i++) {
      if (current[i].second > 0) {
        int gm = current[i].first;
        float value = current[i].second;
        attacked[i] += value;
        //debug_count++;
